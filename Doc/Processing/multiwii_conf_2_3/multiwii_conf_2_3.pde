...
private static final int
  ...
  MSP_SET_RAW_RC_TINY      =150,   //in message          4 rc chan
  MSP_ARM                  =151,
  MSP_DISARM               =152,
  MSP_TRIM_UP              =153,
  MSP_TRIM_DOWN            =154,
...
int  pressedKey = 0;
char pressedKeyCode;
int[] serialRCinit = { 500, 500, 500, 0, 0 };
int[] serialRC = { 500, 500, 500, 0, 0 };
  ...
  if (init_com==1 && graph_on==1) {
    ...
    if( pressedKey == 1 )
    {
      pressedKey = 0;
        if( pressedKeyCode == 'a' )
        {
          if( mwArm == 0 )
          {
            mwArm = 1; 
            println("ARM = 1");
            sendRequestMSP(requestMSP(MSP_ARM));              
          }
          else
          {
            mwArm = 0; 
            println("ARM = 0");          
            sendRequestMSP(requestMSP(MSP_DISARM));              
          }
      }
      ...
      if( mwArm == 0 )
      {
        for( i=0; i<5; i++ )
        {
          serialRC[i] = serialRCinit[i];
        }
      }

      payload = new ArrayList<Character>();
      payload.add(char (serialRC[0]/4) ); 
      payload.add(char (serialRC[1]/4)  );
      payload.add(char (serialRC[2]/4) ); 
      payload.add(char (serialRC[3]/4)  );
      payload.add(char (serialRC[4])  );
  
      sendRequestMSP(requestMSP(MSP_SET_RAW_RC_TINY,payload.toArray( new Character[payload.size()]) ));      
