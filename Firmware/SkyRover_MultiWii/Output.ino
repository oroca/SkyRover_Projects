...
#if defined(SKY_ROVER)
  uint8_t PWM_PIN[8] = {1,2,3,4,6,5,12,12};   //for a quad+: rear,right,left,front
#endif  
...
Serial1.println(NUMBER_MOTOR);

  /****************            mark all PWM pins as Output             ******************/
  for(uint8_t i=0;i<NUMBER_MOTOR;i++) {
    Serial1.println(PWM_PIN[i]);

    pinMode(PWM_PIN[i],OUTPUT);
  }
Serial1.println("start in2");
